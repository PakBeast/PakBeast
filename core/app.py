...
class App(ctk.CTk):
    ...
    def _load_pak(self) -> None:
        file_operations.load_pak(self)
    
    def _populate_tree(self, root: Path) -> None:
        file_operations.populate_tree(self, root)
    
    def _finish_loading(self, pak_name: str):
        file_operations.finish_loading(self, pak_name)
    
    def _loading_failed(self, error: Exception):
        file_operations.loading_failed(self, error)
    
    def _cleanup_temp(self) -> None:
        file_operations.cleanup_temp(self)
    
    def _save_project(self) -> None:
        file_operations.save_project(self)
    ...
    def _on_preview_right_click(self, event):
        from operations.preview_edit_operations import on_preview_right_click
    ...
    def _pack_pak(self) -> None:
        packer.pack_pak(self)
    
    # Settings and Dialogs
    def _show_help_window(self):
        show_about_dialog(self, self.settings)
    ...
    def _open_multi_packer_settings(self) -> None:
        """Switch to Settings tab (Multi-Packer section)."""
        if hasattr(self, 'main_tabs'):
            self.main_tabs.set("Settings")
    
    def _open_color_settings(self) -> None:
        """Switch to Settings tab (Color section)."""
        if hasattr(self, 'main_tabs'):
            self.main_tabs.set("Settings")
